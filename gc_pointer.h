...
Pointer<T,size>::Pointer(const Pointer &ob){
    ...
    it->refcount++;  // increments the reference counter
    addr = ob.addr;
...
bool Pointer<T, size>::collect(){
    ...
    do {
        ...
        for (it = refContainer.begin(); it != refContainer.end(); it++) {
            ...
            if (it->refcount > 0)
                continue;
            // it-> refcount == 0 : remove unused entry
            memfreed = true;
...
T *Pointer<T, size>::operator=(T *t){
    ...
    typename std::list<PtrDetails<T> >::iterator it;
    it = findPtrInfo(addr); 
    it->refcount--;  // decrement the reference count for the memory currently being pointed to 
	//collect();
